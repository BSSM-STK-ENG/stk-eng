...
const Outbound = () => {
  ...
  useEffect(() => {
    ...
    const matchedMaterial = materials.find(
      (material) => material.materialCode === requestedMaterialCode && (material.currentStockQty ?? 0) > 0,
    );
    consumedPrefillKeyRef.current = prefillKey;

    if (!matchedMaterial) {
      setNotice({ tone: 'error', message: '현재 출고 가능한 자재가 아니어서 다시 선택해주세요.' });
      return;
    }
  ...
  const availableMaterials = useMemo(
    () =>
      materials.filter(
        (material) => (material.currentStockQty ?? 0) > 0 || material.materialCode === editingTransaction?.materialCode,
      ),
    [editingTransaction?.materialCode, materials],
  );
  const resolvedMaterial = useMemo(
    () =>
      availableMaterials.find((material) => material.materialCode === materialCode) ??
      availableMaterials.find(
        (material) => material.materialCode.toLowerCase() === materialQuery.trim().toLowerCase(),
      ) ??
      null,
    [availableMaterials, materialCode, materialQuery],
  );
  ...
  const editableAvailableStock =
    currentStock +
    (editingTransaction && editingTransaction.materialCode === resolvedMaterial?.materialCode
      ? editingTransaction.quantity
      : 0);
  const remainingAfterDispatch = editableAvailableStock - safeQuantity;
  const insufficientStock = resolvedMaterial != null && safeQuantity > 0 && safeQuantity > editableAvailableStock;
  ...
  const openRepeatOutbound = (transaction: TransactionResponse) => {
    ...
    setManagerUserId(matchedUser?.id ?? '');
    setQuantity('');
    setNote('');
    setEditingTransaction(null);
    setNotice(null);
  ...
  const openEditOutbound = (transaction: TransactionResponse) => {
    const matchedMaterial = materials.find((m) => m.materialCode === transaction.materialCode);
    const matchedUser = userOptions.find((item) => item.name === (sanitizeInventoryText(transaction.manager) ?? ''));
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!managerUserId) {
      setNotice({ tone: 'error', message: '출고 담당자를 선택해주세요.' });
      return;
    }
    ...
    try {
      ...
      setNotice({
        ...
        message: editingTransaction
          ? `${selectedMaterial.materialName} 출고 내역을 수정했습니다.`
          : `${selectedMaterial.materialName} 출고를 등록했습니다. 예상 잔여 재고 ${Math.max(remainingAfterDispatch, 0).toLocaleString()} EA`,
      });
    } catch (error) {
  ...
  const handleRevert = async (id: number) => {
    if (!window.confirm('이 출고 내역을 되돌리시겠습니까? 되돌리면 해당 수량이 재고로 다시 복구됩니다.')) {
      return;
    }
    setNotice(null);
    try {
  ...
  const handleOutboundTemplateDownload = async () => {
    await downloadCsv(
      [
        {
          자재코드: 'MAT-001',
    ...
    <div className="admin-page">
      <section className="admin-header">
        <div className="admin-header-row">
          <div>
            <p className="admin-kicker">출고</p>
            <h2 className="admin-page-title">출고 관리</h2>
            <p className="admin-page-description">
              등록된 사업장과 사용자 이름을 담당자로 선택해 출고 내역을 조회하고 등록합니다.
            </p>
          </div>
      ...
      <AdminSearchField
        ...
        placeholder="자재명, 코드, 설명, 사업장, 담당자 검색"
              ...
              >
                <div className="min-w-0">
                  ...
                  <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500">
                    ...
                    <span>{sanitizeInventoryText(transaction.manager) ?? '담당자 없음'}</span>
              ...
              <div className="mb-5 flex items-start justify-between gap-4">
                <div>
                  <h3 className="text-lg font-bold text-slate-900">
                    {editingTransaction ? '출고 내역 수정' : '신규 출고 등록'}
                  </h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  >
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-slate-500">
                          {resolvedMaterial.materialCode} · 위치 {formatLocation(resolvedMaterial.location)}
                        </p>
                      ...
                      <span
                        className={`inline-flex h-7 min-w-[108px] shrink-0 self-start items-center justify-center whitespace-nowrap rounded-full px-3 py-1 text-xs font-semibold leading-none ${
                    ...
                    <div className="mt-2.5 flex flex-wrap gap-x-4 gap-y-1.5 text-sm">
                      <span className="font-medium text-slate-600">
                        현재 재고{' '}
                        <strong className="ml-1 text-slate-900">{editableAvailableStock.toLocaleString()} EA</strong>
                      </span>
                      <span className="font-medium text-slate-600">
                        출고 요청 <strong className="ml-1 text-rose-600">{safeQuantity.toLocaleString()} EA</strong>
                      </span>
                      <span className="font-medium text-slate-600">
                        출고 후 예상{' '}
                        <strong className={insufficientStock ? 'ml-1 text-rose-600' : 'ml-1 text-slate-900'}>
                    ...
                    {insufficientStock && (
                      <p className="mt-3 text-sm font-medium text-rose-700">
                        현재 재고보다 많은 수량을 입력했습니다. 수량을 줄이거나 입고를 먼저 반영하세요.
                      </p>
                    )}
                  </div>
                )}
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div>
                    ...
                    {userOptions.length === 0 && (
                      <p className="mt-2 text-sm text-amber-600">
                        이름이 설정된 사용자가 없습니다. 먼저 사용자 관리에서 이름을 입력해주세요.
                      </p>
                    )}
                  </div>
                ...
                <div className="flex justify-end gap-2.5 border-t border-slate-100 pt-4">
                  ...
                  <button
                    type="submit"
                    disabled={
                      submitLoading || insufficientStock || businessUnits.length === 0 || userOptions.length === 0
                    }
                    className="inline-flex h-9 items-center justify-center whitespace-nowrap rounded-lg bg-rose-500 px-4 text-sm font-semibold text-white shadow-sm shadow-rose-500/20 transition-colors hover:bg-rose-600 disabled:cursor-not-allowed disabled:opacity-50"
                  >
          ...
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
            <div className="w-full max-w-md rounded-xl bg-white p-5 shadow-2xl ring-1 ring-black/5">
              ...
              <p className="mb-4 text-xs leading-relaxed text-slate-400">
                엑셀(.xlsx) 또는 CSV(.csv) 형식의 파일을 업로드하세요. 등록된 사업장과 사용자 이름만 사용할 수 있고,
                재고 부족 항목이 있으면 서버에서 거부됩니다.
              </p>
