...
const Outbound = () => {
  ...
  const availableMaterials = useMemo(
    () => materials.filter((material) =>
      (material.currentStockQty ?? 0) > 0 || material.materialCode === editingTransaction?.materialCode,
    ),
  ...
  const resolvedMaterial = useMemo(
    () =>
      ...
      ?? availableMaterials.find((material) => material.materialCode.toLowerCase() === materialQuery.trim().toLowerCase())
      ?? null,
  ...
  const editableAvailableStock = currentStock + (
    ...
    <div className="admin-page">
      ...
      <div className="admin-header-row">
        <div>
          ...
          <p className="admin-page-description">등록된 사업장과 사용자 이름을 담당자로 선택해 출고 내역을 조회하고 등록합니다.</p>
        </div>
                ...
                <div className={`rounded-xl border px-3.5 py-3 ${
                  ...
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-slate-500">{resolvedMaterial.materialCode} · 위치 {formatLocation(resolvedMaterial.location)}</p>
                  ...
                  {insufficientStock && (
                    <p className="mt-3 text-sm font-medium text-rose-700">현재 재고보다 많은 수량을 입력했습니다. 수량을 줄이거나 입고를 먼저 반영하세요.</p>
                  )}
                </div>
              )}
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  ...
                  {userOptions.length === 0 && (
                    <p className="mt-2 text-sm text-amber-600">이름이 설정된 사용자가 없습니다. 먼저 사용자 관리에서 이름을 입력해주세요.</p>
                  )}
                </div>
      ...
      {showUploadModal && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-xl bg-white p-5 shadow-2xl ring-1 ring-black/5">
            ...
            <p className="mb-4 text-xs leading-relaxed text-slate-400">엑셀(.xlsx) 또는 CSV(.csv) 형식의 파일을 업로드하세요. 등록된 사업장과 사용자 이름만 사용할 수 있고, 재고 부족 항목이 있으면 서버에서 거부됩니다.</p>
