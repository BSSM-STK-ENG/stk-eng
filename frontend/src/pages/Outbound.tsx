...
const Outbound = () => {
    ...
    const fetchTransactions = async () => {
        ...
        try {
            ...
            setTransactions((res.data as InventoryTransaction[]).filter(t => t.transactionType === 'OUT').sort((a, b) => b.id - a.id));
        } catch (err) {
            console.error(err);
        } finally {
            setLoading(false);
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            setShowModal(false);
            setMaterialCode(''); setQuantity(''); setBusinessUnit(''); setManager(''); setNote('');
            fetchTransactions();
        } catch {
            alert('출고 실패 (재고부족 등). 자재코드와 수량을 확인해주세요.');
        } finally {
    ...
    const handleDelete = async (id: number) => {
        if (!window.confirm('이 출고 내역을 삭제하시겠습니까?')) return;
        try {
    ...
    return (
        <div className="flex flex-col gap-4 md:gap-5">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                <div>
                    <h2 className="text-xl md:text-2xl font-extrabold tracking-tight text-slate-800">출고 관리</h2>
                    <p className="text-xs md:text-sm text-slate-400 mt-0.5 font-medium">자재를 출고하고 내역을 조회합니다.</p>
                </div>
                <div className="flex flex-wrap gap-2">
                    ...
                    <button onClick={() => setShowModal(true)} className="flex items-center px-3 py-1.5 bg-rose-500 text-white text-xs font-bold rounded-lg shadow-sm shadow-rose-500/20 hover:bg-rose-600 transition-colors">
                        <Plus size={14} className="mr-1.5" /> 신규 출고
                    </button>
                </div>
            </div>
            ...
            <div className="relative max-w-md">
                ...
                <input
                    ...
                    className="w-full pl-9 pr-4 py-2 bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-rose-500/30 focus:border-rose-400 transition-all outline-none text-sm text-slate-700 shadow-sm"
                />
            </div>
                    ...
                    <table className="min-w-full divide-y divide-slate-100">
                        <thead>
                            <tr className="bg-slate-50/80">
                                <th className="px-3 md:px-5 py-3 text-left text-[11px] font-bold text-slate-400 uppercase tracking-wider">출고 날짜</th>
                                ...
                                <th className="px-3 md:px-5 py-3 text-left text-[11px] font-bold text-slate-400 uppercase tracking-wider hidden lg:table-cell">담당자</th>
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {paged.map((t) => (
                                <tr key={t.id} className="hover:bg-rose-50/30 transition-colors group">
                                    ...
                                    <td className="px-3 md:px-5 py-3 whitespace-nowrap text-xs md:text-sm text-right font-extrabold text-rose-500">-{t.quantity}</td>
                                    <td className="px-3 md:px-5 py-3 whitespace-nowrap text-xs text-slate-400 hidden lg:table-cell">{t.manager || '-'}</td>
            ...
            {showModal && createPortal(
                <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={() => setShowModal(false)}>
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 ring-1 ring-black/5" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between mb-5">
                            <h3 className="text-lg font-extrabold text-slate-800">신규 출고 등록</h3>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                ...
                                <input type="text" required value={materialCode} onChange={e=>setMaterialCode(e.target.value)} className="w-full px-3 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-rose-500/30 focus:border-rose-400 outline-none text-sm font-medium" />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-slate-500 mb-1.5 uppercase tracking-wider">수량</label>
                                <input type="number" required min="1" value={quantity} onChange={e=>setQuantity(e.target.value)} className="w-full px-3 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-rose-500/30 focus:border-rose-400 outline-none text-sm font-medium" />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-slate-500 mb-1.5 uppercase tracking-wider">사업장</label>
                                <input type="text" value={businessUnit} onChange={e=>setBusinessUnit(e.target.value)} className="w-full px-3 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-rose-500/30 focus:border-rose-400 outline-none text-sm font-medium" />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-slate-500 mb-1.5 uppercase tracking-wider">출고 담당자</label>
                                <input type="text" value={manager} onChange={e=>setManager(e.target.value)} className="w-full px-3 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-rose-500/30 focus:border-rose-400 outline-none text-sm font-medium" />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-slate-500 mb-1.5 uppercase tracking-wider">비고</label>
                                <input type="text" value={note} onChange={e=>setNote(e.target.value)} className="w-full px-3 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-rose-500/30 focus:border-rose-400 outline-none text-sm font-medium" />
                            </div>
                            <div className="flex justify-end gap-2.5 pt-4 border-t border-slate-100">
                                <button type="button" onClick={() => setShowModal(false)} className="px-4 py-2 text-sm text-slate-500 font-bold hover:bg-slate-50 rounded-xl transition-colors">취소</button>
                                <button type="submit" disabled={submitLoading} className="px-5 py-2 bg-rose-500 text-white text-sm font-bold rounded-xl hover:bg-rose-600 transition-colors shadow-sm shadow-rose-500/20 disabled:opacity-50">
                ...
                <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={() => setShowUploadModal(false)}>
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 ring-1 ring-black/5" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-lg font-extrabold text-slate-800 flex items-center"><FileSpreadsheet size={20} className="mr-2 text-emerald-500" />출고 일괄 업로드</h3>
                            <button onClick={() => setShowUploadModal(false)} className="p-1 rounded-lg hover:bg-slate-100 text-slate-400"><X size={18} /></button>
                        </div>
                        <p className="text-xs text-slate-400 mb-4">엑셀(.xlsx) 또는 CSV(.csv) 형식을 업로드하세요.</p>
                        <form onSubmit={handleFileUpload} className="space-y-4">
                            <div className="border-2 border-dashed border-slate-200 rounded-xl p-6 text-center hover:border-emerald-300 transition-colors">
                                ...
                                <input type="file" accept=".xlsx,.xls,.csv" required onChange={(e: React.ChangeEvent<HTMLInputElement>) => setUploadFile(e.target.files?.[0] ?? null)} className="w-full text-xs text-slate-500 file:mr-3 file:py-1.5 file:px-3 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-emerald-50 file:text-emerald-600 hover:file:bg-emerald-100 cursor-pointer" />
