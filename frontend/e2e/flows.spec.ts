...
test.describe('Key flows', () => {
  test('dashboard cards navigate to filtered current stock views', async ({ page }) => {
    ...
    await expect(page.locator('.admin-table-panel').getByRole('button', { name: /재고 없음/ })).toHaveClass(/bg-slate-900/);
    ...
    await expect(page.locator('.admin-table-panel').getByRole('button', { name: /안전재고 이하/ })).toHaveClass(/bg-slate-900/);
  });
